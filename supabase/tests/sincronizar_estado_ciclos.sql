...
select closes_at from sigav.survey_applications
where id = '00000000-0000-4000-8000-00000000e102';

select sigav.fc_sincronizar_estado_ciclos();

select is(
  (select status from sigav.survey_applications where id = '00000000-0000-4000-8000-00000000e101'),
  'OPEN',
  'caso 1 — aberto com prazo no futuro continua OPEN'
);

select is(
  (select status from sigav.survey_applications where id = '00000000-0000-4000-8000-00000000e102'),
  'CLOSED',
  'caso 2 — aberto com prazo vencido passa a CLOSED'
);

select is(
  (select status from sigav.survey_applications where id = '00000000-0000-4000-8000-00000000e103'),
  'OPEN',
  'caso 3 — agendado dentro da janela passa a OPEN'
);

select is(
  (select status from sigav.survey_applications where id = '00000000-0000-4000-8000-00000000e104'),
  'CLOSED',
  'caso 4 — agendado com prazo vencido passa a CLOSED sem abrir antes'
);

select is(
  (select status from sigav.survey_applications where id = '00000000-0000-4000-8000-00000000e105'),
  'CLOSED',
...
select is(
  (select status || ' | ' || closes_at::text from sigav.survey_applications
   where id = '00000000-0000-4000-8000-00000000e102'),
  (select 'CLOSED | ' || closes_at::text from prazo_original),
...
select is(
  (select count(*)::integer from sigav.audit_events
   where application_id = '00000000-0000-4000-8000-00000000e104'
     and event_type = 'SURVEY_CYCLE_AUTO_OPEN'),
  0,
...
select is(
  (select count(*)::integer from sigav.audit_events
   where event_type = 'SURVEY_CYCLE_AUTO_CLOSE'
     and application_id in ('00000000-0000-4000-8000-00000000e102', '00000000-0000-4000-8000-00000000e104')),
  2,
...
select is(
  (select metadata ->> 'reason' from sigav.audit_events
   where event_type = 'SURVEY_CYCLE_AUTO_CLOSE'
     and application_id = '00000000-0000-4000-8000-00000000e102'),
  'closes_at_reached',
  'a auditoria diz por que o ciclo fechou'
);

select is(
  (select before_data ->> 'applicationStatus' from sigav.audit_events
   where event_type = 'SURVEY_CYCLE_AUTO_CLOSE'
     and application_id = '00000000-0000-4000-8000-00000000e104'),
  'SCHEDULED',
