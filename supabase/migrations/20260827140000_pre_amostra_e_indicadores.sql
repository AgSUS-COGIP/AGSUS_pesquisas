...
as $$
  select sigav.can_manage_surveys()
    ...
    or exists (
      select 1
      from sigav.survey_applications sa
      join sigav.application_participants ap on ap.application_id = sa.id
      where sa.id = target_application_id
        and ap.person_id = sigav.current_person_id()
...
create or replace function sigav.fc_configurar_pre_amostra(
...
begin
  ...
  if exists (select 1 from sigav.submissions where application_id = v_application.id) then
  ...
  if v_mode = 'RANDOM' then
    ...
    if exists (
      ...
      where not exists (
        select 1 from sigav.application_participants ap
  ...
  update sigav.application_participants
  ...
  insert into sigav.audit_events(actor_person_id, event_type, entity_type, entity_id, application_id, after_data, metadata)
...
create or replace function sigav.fc_proteger_pre_amostra()
returns trigger
language plpgsql
...
drop trigger if exists tbu_proteger_pre_amostra on sigav.survey_applications;
create trigger tbu_proteger_pre_amostra before update of status on sigav.survey_applications
for each row execute function sigav.fc_proteger_pre_amostra();

create or replace function sigav.fc_abrir_pre_amostra(target_survey_id uuid)
returns jsonb
language plpgsql
security definer
set search_path = pg_catalog, sigav, auth
...
begin
  if not sigav.can_manage_surveys() then raise exception 'Acesso restrito à administração.'; end if;
  select a.* into v_application
  from sigav.survey_applications a join sigav.survey_versions v on v.id = a.survey_version_id
  where v.survey_id = target_survey_id
  ...
  v_result := sigav.manage_survey_cycle(target_survey_id, 'OPEN', null, null);
...
create or replace function sigav.fc_publicar_populacao(target_survey_id uuid)
returns jsonb
language plpgsql
security definer
set search_path = pg_catalog, sigav, auth
...
begin
  ...
  insert into sigav.audit_events(actor_person_id, event_type, entity_type, entity_id, application_id, before_data, after_data, metadata)
...
create or replace function sigav.fc_obter_pre_amostra(target_survey_id uuid)
returns jsonb
language plpgsql
security definer
set search_path = pg_catalog, sigav, auth
...
begin
  ...
  into v_population, v_size from sigav.application_participants
  where application_id = v_application.id and status not in ('BLOCKED', 'EXCLUDED');
  select count(*)::integer into v_submitted from sigav.submissions
  ...
  return jsonb_build_object('status', 'OK', 'applicationId', v_application.id,
    ...
    'participantIds', coalesce((select jsonb_agg(ap.id order by p.full_name)
      from sigav.application_participants ap join sigav.people p on p.id = ap.person_id
...
create or replace function sigav.fc_marcar_grupo_amostra()
returns trigger
language plpgsql
security definer
set search_path = pg_catalog, sigav, auth
...
begin
  ...
  where ap.application_id = new.application_id and ap.person_id = sigav.current_person_id()
...
drop trigger if exists tbi_marcar_grupo_amostra on sigav.submissions;
create trigger tbi_marcar_grupo_amostra before insert on sigav.submissions
for each row execute function sigav.fc_marcar_grupo_amostra();

create or replace function sigav.fc_obter_matriz_pre_amostra(target_survey_id uuid)
returns jsonb
language plpgsql
security definer
set search_path = pg_catalog, sigav, auth
as $$
declare v_application sigav.survey_applications%rowtype; v_items jsonb; v_rows jsonb;
begin
  if not sigav.can_manage_surveys() then raise exception 'Acesso restrito à administração.'; end if;
  select a.* into v_application
  from sigav.survey_applications a join sigav.survey_versions v on v.id = a.survey_version_id
  ...
  into v_items
  from sigav.survey_questions q join sigav.survey_sections sec on sec.id = q.section_id
  where q.survey_version_id = v_application.survey_version_id and (
    ...
    or (q.question_type = 'SINGLE_CHOICE' and exists (
      select 1 from sigav.question_options qo where qo.question_id = q.id and qo.active and qo.score is not null))
  );
  ...
  from (
    select s.submitted_at, coalesce(jsonb_object_agg(q.id::text, to_jsonb(coalesce(
      a.answer_number,
      (select qo.score from sigav.answer_options ao join sigav.question_options qo on qo.id = ao.option_id
    ...
    cross join sigav.survey_questions q
    left join sigav.answers a on a.submission_id = s.id and a.question_id = q.id
...
revoke all on function sigav.fc_configurar_pre_amostra(uuid, text, integer, uuid[]) from public, anon;
revoke all on function sigav.fc_abrir_pre_amostra(uuid) from public, anon;
revoke all on function sigav.fc_publicar_populacao(uuid) from public, anon;
revoke all on function sigav.fc_obter_pre_amostra(uuid) from public, anon;
revoke all on function sigav.fc_obter_matriz_pre_amostra(uuid) from public, anon;
revoke all on function sigav.fc_marcar_grupo_amostra() from public, anon, authenticated;
revoke all on function sigav.fc_proteger_pre_amostra() from public, anon, authenticated;
grant execute on function sigav.fc_configurar_pre_amostra(uuid, text, integer, uuid[]) to authenticated;
grant execute on function sigav.fc_abrir_pre_amostra(uuid) to authenticated;
grant execute on function sigav.fc_publicar_populacao(uuid) to authenticated;
grant execute on function sigav.fc_obter_pre_amostra(uuid) to authenticated;
grant execute on function sigav.fc_obter_matriz_pre_amostra(uuid) to authenticated;
