...
--
-- O que fica aqui é o **registro** da mudança, e `sigav.audit_events` já tem
...
--
-- `audit_events` não aceita escrita de `authenticated` — e não deve aceitar,
...
create or replace function sigav.fc_registrar_manutencao_auditoria(
  p_evento text,
  p_motivo text,
...
revoke execute on function sigav.fc_registrar_manutencao_auditoria(text, text, jsonb, jsonb, text[]) from public, anon;
grant execute on function sigav.fc_registrar_manutencao_auditoria(text, text, jsonb, jsonb, text[]) to authenticated;

-- Rollback:
-- begin;
--   drop function if exists sigav.fc_registrar_manutencao_auditoria(text, text, jsonb, jsonb, text[]);
-- commit;
