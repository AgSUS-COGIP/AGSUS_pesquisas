...
-- Fundação do modelo de permissões por módulo.
--
...
delete from public.role_module_permissions rmp
...
from public.system_roles sr
cross join public.platform_modules pm
...
as $function$
  ...
  module_state as (
    ...
    left join public.system_roles sr on sr.code = er.code
    left join public.role_module_permissions rmp
      on rmp.role_id = sr.id
     and rmp.module_code = pm.code
    left join public.person_module_permissions pmp
...
-- Helper público para RPCs/RLS de domínio. A função sempre verifica a pessoa da
-- sessão atual; não recebe person_id, evitando transformar a autorização em uma
-- API de consulta de permissões de terceiros.
create or replace function public.has_platform_module(target_module_code text)
returns boolean
language sql
stable
security definer
...
as $function$
  select coalesce(
    upper(btrim(coalesce(target_module_code, ''))) = any(
      private.effective_platform_modules(public.current_person_id())
    ),
    false
  );
$function$;

revoke all on function public.has_platform_module(text) from public, anon;
grant execute on function public.has_platform_module(text) to authenticated;
...
comment on function public.has_platform_module(text) is
