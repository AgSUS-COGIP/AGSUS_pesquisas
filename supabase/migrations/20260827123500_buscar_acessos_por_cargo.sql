...
begin
  ...
  into v_roles
  from sigav.system_roles as role;
  ...
  select coalesce(jsonb_agg(jsonb_build_object(
    ...
    'roles', coalesce((
      ...
      from sigav.person_role_assignments as assignment
      join sigav.system_roles as role on role.id = assignment.role_id
  ...
  from (
    select candidate.*
    from sigav.people as candidate
