...
`20260812090000_restaurar_validacao_integridade_ciclo.sql` corrige uma regressão introduzida por `20260811120000`: essa migration foi escrita a partir de uma versão de `manage_survey_cycle` anterior a `20260804195030_validate_survey_integrity_before_publish.sql`, e reverteu sem intenção a chamada a `validate_survey_version_integrity()` que bloqueava `SCHEDULE`, `OPEN` e `REOPEN` com pendência `BLOCKING`. A nova migration redefine `manage_survey_cycle` a partir da definição vigente, restaurando essa chamada nos três branches — `PUBLISH` fica de fora, mantendo a checagem rasa de seções/perguntas que já tinha. `REOPEN` também deixou de exigir abertura digitada: a tela envia `target_opens_at = null` e o branch resolve `coalesce(target_opens_at, now())` com o relógio do banco — decidir "agora" no navegador deixava o resultado refém do relógio da estação do operador (adiantado, gravaria `SCHEDULED` em silêncio); o client só valida a nova data de encerramento. Redefine função legada consumida por bundle publicado, então tem entrada em `LEGACY_RESTORED_OBJECTS` (ver [../scripts/CLAUDE.md](../scripts/CLAUDE.md)).
