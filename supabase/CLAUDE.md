...
**A fila tem quatro estados, e o repositório demorou a saber disso.** `PENDENTE` (na fila, ninguém tentou) → `PROCESSANDO` (reivindicado) → `ENVIADO` ou `FALHOU`. O banco de produção já usava `PENDENTE` por SQL aplicado **sem migration** — divergência descoberta em 20/08/2026, e que produzia um defeito mudo: `fc_agendar_envio_manual` inseria com o default do banco (`PENDENTE`) enquanto a reivindicação procurava `PROCESSANDO`, então **envio dirigido entrava na fila e nunca saía**, com o despacho respondendo "0 enviados" como se não houvesse trabalho. `20260820160000` adotou o estado em vez de removê-lo: a distinção é útil, e o claim virou literal (`update … for update skip locked`, que é o que impede envio em dobro quando a tela e o cron rodam juntos).

**O claim expira em 15 minutos** (`20260820180000`). `PROCESSANDO` significa "alguém reivindicou", não "vai sair": se o despacho morre entre o claim e o envio — ou nem chega a rodar, num ambiente sem `SMTP_APP_PASSWORD` —, a linha ficaria reivindicada para sempre, e a proteção contra clique duplo de `fc_agendar_envio_manual` viraria impedimento permanente. `fc_liberar_emails_travados()` devolve à fila o que passou do prazo e é chamada no início de cada despacho. O prazo é folgado: um lote é curto por construção (40 mensagens, 20 s), então nenhum claim honesto chega perto.
