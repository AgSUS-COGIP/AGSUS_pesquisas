...
calculados no PostgreSQL.

## Fonte de verdade

Desde `20260826193000_fundar_permissoes_por_modulo.sql`:

- `system_roles` contém os quatro perfis institucionais;
- `role_module_permissions` define o pacote padrão de módulos de cada perfil;
- `person_module_permissions` aplica concessão ou bloqueio individual;
- `private.effective_platform_modules(person_id)` calcula o conjunto efetivo;
- `has_platform_module(module_code)` é o helper de autorização para RPCs e RLS;
- `fc_obter_contexto_plataforma()` devolve os módulos efetivos em `modules`;
...
`person_module_permissions` possui uma linha por pessoa e módulo:
...
`person_role_assignments (person_id) where ends_at is null`.

Toda RPC que altera perfil deve encerrar a atribuição vigente antes de criar a
nova. `fc_definir_perfil_pessoa` já segue essa ordem e impede o Superadmin de
retirar o próprio perfil.
...
```sql
if not public.has_platform_module('ADMIN_TEAMS') then
...
`is_platform_administrator()` continua válido para operações que realmente
...
left join public.role_module_permissions rmp on rmp.role_id = sr.id
left join public.platform_modules pm on pm.code = rmp.module_code
group by sr.code
order by sr.code;

select person_id, module_code, allowed, updated_at
from public.person_module_permissions
...
6. `fc_obter_contexto_plataforma()` retorna `modules` coerente com o perfil;
...
9. `has_platform_module()` produz o mesmo resultado do contexto.
...
devolve `modules`, mas o `CASE` legado do banco não corresponde integralmente ao
mapa vigente do frontend para Admin.

Portanto a ordem segura é:

1. aplicar a migration `20260826193000_fundar_permissoes_por_modulo.sql`;
2. validar `fc_obter_contexto_plataforma()` com os quatro perfis;
...
Remover ou renomear uma RPC usada por bundles já publicados pode derrubar a
plataforma. Quando houver substituição de função, mantenha uma ponte durante a
janela de transição e remova-a apenas após confirmar o bundle novo em produção.

O PR 0A não remove nenhuma RPC existente: ele substitui o corpo de
`fc_obter_contexto_plataforma()` preservando assinatura e contrato JSON.

## Rollback do PR 0A

O rollback precisa restaurar duas coisas em conjunto:

1. o mapa anterior em `role_module_permissions`;
2. o corpo anterior de `fc_obter_contexto_plataforma()` que derivava módulos por perfil.
...
`person_module_permissions` não é apagada pelo rollback: overrides são dados de
segurança e devem ser preservados para investigação ou restauração controlada.
