...
export default function AccessPage() {
  ...
  async function signInWithGoogle() {
    ...
    try {
      ...
      const supabase = createBrowserSupabaseClient();
      const redirectTo = `${getCallbackBaseUrl()}/auth/confirm?next=${encodeURIComponent(next)}`;
  ...
  return (
    <main className="relative flex min-h-screen items-center justify-center overflow-hidden bg-[#123f50] px-5 py-10 text-[#10243e]">
      ...
      {background && <div className={`absolute inset-0 bg-cover bg-center transition-opacity duration-700 ${backgroundVisible ? "opacity-100" : "opacity-0"}`} style={{ backgroundImage: `url(${background})` }} />}
