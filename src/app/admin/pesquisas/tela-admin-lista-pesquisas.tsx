...
export default function AdminSurveysPage() {
  ...
  return <PlatformShell user={guard.user} eyebrow="Administração" title="Avaliações e ciclos" actions={<Link href="/admin/pesquisas/nova" className="primary-button"><FilePlus2 className="h-4 w-4" />Nova avaliação</Link>}>
    <section className="rounded-2xl border border-[var(--border-subtle)] border-t-[3px] border-t-[var(--brand-solid)] bg-[var(--surface-card)] p-5 shadow-sm sm:p-6">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
        <div className="max-w-2xl">
          <span className="inline-flex items-center gap-2 rounded-full border border-[var(--border-subtle)] bg-[var(--surface-interactive)] px-3 py-1 text-[11px] font-black uppercase tracking-[.12em] text-[var(--brand-secondary)]"><Sparkles className="h-3.5 w-3.5" />Estúdio e operação</span>
          <h2 className="mt-3 text-2xl font-black tracking-tight text-[var(--text-primary)]">Da construção à abertura do ciclo</h2>
          <p className="mt-2 text-sm leading-6 text-[var(--text-secondary)]">Edite o instrumento, valide a prontidão, configure o período e controle a publicação em um fluxo auditável.</p>
        </div>
        <Link href="/admin/pesquisas/nova" className="primary-button shrink-0"><FilePlus2 className="h-4 w-4" />Criar avaliação</Link>
      </div>
    </section>

    <section className="mt-6 grid gap-4 sm:grid-cols-3">
      {[["Avaliações", surveys.length], ["Perguntas cadastradas", totalQuestions], ["Ciclos ativos", activeCycles]].map(([label, value]) => <article key={String(label)} className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm"><p className="text-xs font-black uppercase tracking-[.14em] text-slate-400">{label}</p><strong className="mt-2 block text-3xl font-black text-[var(--brand-primary)]">{value}</strong></article>)}
    </section>

    <section className="mt-6 rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between"><div><h3 className="text-xl font-black text-[var(--brand-primary)]">Catálogo administrativo</h3><p className="mt-1 text-sm text-slate-500">Construa o formulário ou gerencie o ciclo de cada avaliação.</p></div><label className="relative block w-full max-w-sm"><Search className="absolute left-4 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-400" /><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Buscar por código, nome ou ciclo" className="w-full rounded-2xl border border-slate-200 bg-slate-50 py-3 pl-11 pr-4 font-semibold outline-none focus:border-blue-400 focus:bg-white focus:ring-4 focus:ring-blue-100" /></label></div>
      ...
      <div className="mt-6 grid gap-4 xl:grid-cols-2">
        {dataLoading ? Array.from({ length: 4 }).map((_, index) => <div key={index} className="h-72 animate-pulse rounded-3xl bg-slate-100" />) : filtered.length ? filtered.map((survey) => <article key={survey.surveyId} className="rounded-3xl border border-slate-200 p-5 transition hover:border-blue-200 hover:shadow-lg">
          <div className="flex items-start justify-between gap-4"><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><span className="rounded-lg bg-blue-50 px-2.5 py-1 text-xs font-black text-[var(--brand-primary)]">{survey.code}</span><span className={`rounded-full px-3 py-1 text-xs font-black ${statusClass(survey.applicationStatus ?? survey.status)}`}>{survey.applicationStatus ?? survey.status}</span></div><h4 className="mt-4 text-xl font-black text-[var(--brand-primary)]">{survey.name}</h4><p className="mt-2 line-clamp-2 text-sm leading-6 text-slate-500">{survey.description || "Sem descrição cadastrada."}</p></div><div className="grid h-12 w-12 shrink-0 place-items-center rounded-2xl bg-slate-100 text-[var(--brand-primary)]"><FileQuestion className="h-6 w-6" /></div></div>
          <div className="mt-5 grid grid-cols-3 gap-3">{[["Versão", survey.versionNumber], ["Seções", survey.sections], ["Perguntas", survey.questions]].map(([label, value]) => <div key={String(label)} className="rounded-2xl bg-slate-50 p-3 text-center"><span className="block text-[10px] font-black uppercase tracking-[.12em] text-slate-400">{label}</span><strong className="mt-1 block text-lg text-[var(--brand-primary)]">{value}</strong></div>)}</div>
          ...
          <div className="mt-5 grid gap-3 sm:grid-cols-2"><Link href={`/admin/pesquisas/${survey.surveyId}`} className="inline-flex items-center justify-center gap-2 rounded-xl border border-slate-200 bg-white px-4 py-3 text-sm font-black text-[var(--brand-primary)] hover:bg-slate-50"><Settings2 className="h-4 w-4" />Construir formulário</Link><Link href={`/admin/pesquisas/${survey.surveyId}/operacao`} className="primary-button justify-center"><SlidersHorizontal className="h-4 w-4" />Operar ciclo</Link></div>
        </article>) : <div className="col-span-full rounded-3xl border border-dashed border-slate-300 p-12 text-center"><FileQuestion className="mx-auto h-10 w-10 text-slate-300" /><strong className="mt-4 block text-[var(--brand-primary)]">Nenhuma avaliação encontrada</strong><p className="mt-2 text-sm text-slate-500">Crie o primeiro instrumento da plataforma.</p></div>}
      </div>
