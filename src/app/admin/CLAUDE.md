...
| `/admin/pesquisas/[surveyId]` | `pesquisas/[surveyId]/tela-admin-construtor-pesquisa.tsx` | `ADMIN_SURVEYS` | `/api/avaliacoes/[id]/construtor`, `…/secoes`, `…/perguntas`, `…/itens/copia`, `…/itens/ordem`, `…/regras` |
...
Oito decisões estruturam a tela:

- **O agendamento mora no cartão de período, não na fileira de operações.** "Agendar abertura" era um botão sem dado próprio — toda a informação dele estava no campo **Abertura**, dois cartões acima, e a distância entre os dois fazia o par parecer redundante. Hoje o botão primário do cartão de período é contextual, como já era com `REOPEN`: **Reabrir ciclo com este período** (ciclo encerrado), **Salvar e agendar abertura** (`readyToOpen`, ciclo parado e abertura no futuro, por `opensInFuture()`) ou **Salvar período**. É **uma** chamada, não duas: `SCHEDULE` passou a aceitar `target_opens_at`/`target_closes_at` — encadear `UPDATE_PERIOD` e depois `SCHEDULE` não seria atômico e deixaria o ciclo com período novo e sem agendamento se a segunda falhasse. O bloco de datas registradas ganhou a linha **"O que acontece"** (`periodOutcome()`), que diz a consequência em vez de repetir os campos. `OPEN` virou **"Abrir agora"**: com a abertura automática, ele deixou de ser o gêmeo do agendamento e passou a ser a antecipação deliberada dela.
- **Nenhum botão fica apagado sem explicação.** Cada operação do ciclo de vida é um objeto `CycleAction` com `description` (o que a ação faz) e `blockedReason` (por que está indisponível), derivado do estado atual. `description` vira tooltip revelado no hover/foco do próprio botão (`ActionButton`, sem selo "i" separado), **inclusive quando o botão está desativado** — saber o que a ação faz não depende de ela estar disponível agora. `blockedReason` **nunca** vai para tooltip: as ações bloqueadas são reunidas em `blockedActions` e listadas logo abaixo da fileira, uma linha por ação, sempre visíveis e nomeando a ação a que se referem. Adicionar operação nova exige preencher os dois textos. Os dois textos têm `id` próprio (`acao-<AÇÃO>-descricao` e `acao-<AÇÃO>-nota`) e o `aria-describedby` do botão aponta para os dois quando há bloqueio.
- **A explicação de bloqueio fica numa lista abaixo da fileira, não sob cada botão.** Tentar ancorá-la a cada botão produzia uma frase mais larga que o próprio botão, que invadia o vizinho e deixava a fileira com alturas desiguais. Numa lista única, os botões mantêm altura igual e cada motivo continua ligado ao seu botão por `aria-describedby`.
- **O balão é ancorado pela borda esquerda do botão, e essa não é escolha estética.** `.platform-shell-content` é `position: fixed` com `overflow-x: hidden` (`src/app/sidebar-monitora.css`), o que o torna um **contexto de empilhamento próprio**: nenhum `z-index` de dentro do conteúdo vence a barra lateral, que é `z-50` na raiz — e o que passa da borda ainda é recortado. Centralizado (`left-1/2 -translate-x-1/2`), o balão de um botão à esquerda avançava para fora e aparecia cortado sob a barra. Com `left-0` ele só cresce para a direita. **Subir o `z-index` não resolve esse caso** — é a solução que parece óbvia e não funciona.
...
- **Texto de apoio vai para tooltip revelado no hover/foco, não para um parágrafo sempre visível.** A explicação de "Avisos por e-mail" era um parágrafo fixo de duas frases; hoje é um selo "i" ao lado do `h3`, com o balão de `@/components/ui/tooltip` (`InfoTooltip`). O mesmo vale para os hints de **Abertura**/**Encerramento** em `PeriodField`, que trocaram a linha de texto sob o rótulo pelo mesmo selo, e para a `description` de cada operação do ciclo de vida (`ActionButton`) — ali não há selo "i" à parte: o botão inteiro é a área de hover/foco que revela o balão. A **única** exceção continua sendo `blockedReason`: a explicação de por que um botão está indisponível permanece sempre visível, na lista abaixo da fileira de botões — é a decisão de cima ("Nenhum botão fica apagado sem explicação"), que existe para quem não passa o mouse nem usa teclado para explorar, e um tooltip que só abre no hover a esconderia de novo. Os quatro cartões de métrica (Estrutura, Participantes, Em preenchimento, Respostas enviadas) também deixaram de ser cartões soltos com borda e sombra próprias — hoje são células de uma única faixa (`gap-px` sobre `--border-subtle` desenha os divisores), porque cada um só tinha um número e uma legenda curta para o peso visual de quatro cartões repetidos.
...
- **Cartão "Ciclo de vida" / "Operações disponíveis".** O `Surface` que envolvia a grade de `ActionCard` foi removido — as operações (`Publicar versão`, `Abrir agora`, `Interromper avaliação`, `Criar nova versão`) hoje são só uma fileira solta de botões (`ActionButton`), sem eyebrow, sem `h3` e sem card em volta. O aviso "Publicação protegida" (quando o checklist bloqueia a publicação) continua acima da fileira, também fora de qualquer card.
...
**O construtor não tem mais seção de exclusão.** Existiu uma zona destrutiva ao fim da página, visível só com a versão em `DRAFT`, chamando `fc_excluir_pesquisa_rascunho` — removida por ser redundante com o caminho que já existe no catálogo (`/admin/pesquisas`): **Arquivar** seguido de **Apagar definitivamente**. Esse caminho é estritamente mais amplo — `fc_excluir_pesquisa_arquivada` (usada por "Apagar definitivamente") apaga a pesquisa inteira, todas as versões, ciclos e submissões, bastando que esteja arquivada, enquanto `fc_excluir_pesquisa_rascunho` só aceitava um rascunho nunca publicado e sem nenhuma resposta. A RPC `fc_excluir_pesquisa_rascunho` e a rota `DELETE /api/avaliacoes/[id]` (sem `?arquivada=true`) continuam existindo no banco — não foram removidas nesta limpeza, que ficou restrita à interface — mas não têm mais chamador na aplicação.
