...
**Notificações por e-mail aos participantes.** A tela tem um cartão próprio com o checkbox "Enviar e-mails aos participantes" (`st_notificacao_email` no ciclo, persistido por `fc_definir_notificacao_email`). Ligado, cada participante recebe um e-mail na abertura do ciclo e outro nas 24 horas finais — cada um no máximo uma vez, garantido pela chave única de `tl_email_participante`. Quem decide quem recebe é `fc_reivindicar_emails()` (banco); quem envia é `/api/tarefas/emails` (cron da Vercel + disparo imediato via `after()` quando o operador abre o ciclo). Sem participantes vinculados o checkbox não liga, e o motivo aparece junto. O envio é por SMTP institucional (`@/config/email`), não por provedor de terceiro. Detalhes de configuração (`SMTP_APP_PASSWORD`, `CRON_SECRET`) em [../api/CLAUDE.md](../api/CLAUDE.md).
