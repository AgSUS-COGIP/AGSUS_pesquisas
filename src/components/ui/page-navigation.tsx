        ...
        {items.map((item, index) => {
          ...
          return (
            <li key={`${item.label}-${index}`} className="flex items-center gap-1.5">
              ...
              {item.href && !current ? (
                <Link href={item.href} className="rounded-md px-1 py-0.5 transition hover:text-[var(--brand-primary)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-200">
                  {item.label}
                </Link>
              ) : (
                <span aria-current={current ? "page" : undefined} className={cn("px-1 py-0.5", current && "text-slate-800")}>
                  {item.label}
                </span>
              )}
            </li>
