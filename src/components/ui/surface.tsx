...
export function Surface({ className, ...props }: HTMLAttributes<HTMLElement>) {
  return <section className={cn("rounded-xl border border-[var(--border-subtle)] bg-white shadow-[var(--shadow-card)]", className)} {...props} />;
}
...
export function StatCard({ label, value, description, className }: { label: string; value: ReactNode; description?: string; className?: string }) {
  return (
    <article className={cn("rounded-xl border border-[var(--border-subtle)] bg-white p-5", className)}>
