    ...
    <section className="mt-6" aria-label={title}>
      ...
      <nav className="mt-2 space-y-1.5">
        {allowed.map((item) => {
          ...
          return (
            <Link key={item.href} href={item.href} onClick={onNavigate} title={compact ? item.label : undefined} aria-current={active ? "page" : undefined} className={`group flex items-center gap-3 rounded-xl px-3 py-3 text-sm font-extrabold transition ${active ? "bg-white text-[#003b70] shadow-sm" : "text-blue-50 hover:bg-white/10"}`}>
...
function Sidebar({ user, compact, modules, mobile, onNavigate, onToggle, onSignOut }: { user: PlatformUser; compact: boolean; modules: string[]; mobile?: boolean; onNavigate?: () => void; onToggle?: () => void; onSignOut: () => void }) {
  return (
    <aside className={`${mobile ? "fixed inset-y-0 left-0 z-[70] flex w-72" : `fixed inset-y-0 left-0 z-50 hidden lg:flex ${compact ? "w-20" : "w-64"}`} flex-col bg-[linear-gradient(180deg,#003b70,#002c54)] text-white shadow-2xl transition-all`}>
      ...
      {!mobile && <button type="button" onClick={onToggle} className="absolute -right-4 top-24 grid h-8 w-8 place-items-center rounded-full border border-blue-200 bg-white text-[#003b70] shadow-md transition hover:scale-105" aria-label={compact ? "Expandir menu" : "Recolher menu"}><PlatformIcon name={compact ? "chevron-right" : "chevron-left"} className="h-4 w-4" /></button>}
      ...
      <div className="border-t border-white/10 p-3"><div className={`rounded-2xl border border-white/15 bg-white/10 p-3 ${compact && !mobile ? "text-center" : ""}`}>
        <Link href="/perfil" onClick={onNavigate} className={`flex items-center gap-3 rounded-xl ${compact && !mobile ? "justify-center" : ""}`}><Avatar user={user} size="sm" />{(!compact || mobile) && <div className="min-w-0"><strong className="block truncate text-sm">{user.fullName}</strong><span className="text-xs text-blue-200">{user.profileLabel}</span></div>}</Link>
        <button type="button" onClick={onSignOut} className={`mt-3 flex w-full items-center justify-center gap-2 rounded-xl border border-white/20 px-3 py-2 text-sm font-bold transition hover:bg-white/10 ${compact && !mobile ? "px-2" : ""}`}><PlatformIcon name="logout" className="h-4 w-4" />{(!compact || mobile) && "Sair do sistema"}</button>
      </div></div>
    </aside>
  );
}
      ...
      <div className={`transition-all ${compact ? "lg:pl-20" : "lg:pl-64"}`}>
        <header className="sticky top-0 z-40 border-b border-[#d7e5f2] bg-white/95 px-4 py-3 shadow-sm backdrop-blur lg:px-7 lg:py-4">
          <div className="mx-auto flex max-w-[1500px] items-center justify-between gap-4">
            <div className="flex min-w-0 items-center gap-3"><button type="button" onClick={() => setMobileOpen(true)} className="grid h-10 w-10 place-items-center rounded-xl border border-slate-200 bg-white text-[#003b70] lg:hidden" aria-label="Abrir menu"><PlatformIcon name="menu" /></button><div className="min-w-0">{eyebrow && <p className="text-[10px] font-black uppercase tracking-[0.18em] text-[#0b8f58]">{eyebrow}</p>}<h1 className="truncate text-xl font-black text-[#003b70] sm:text-2xl">{title}</h1></div></div>
            <div className="flex items-center gap-3"><PlatformCommandMenu modules={modules} />{actions}<Link href="/perfil" className="hidden items-center gap-3 rounded-2xl border border-slate-200 bg-slate-50 px-3 py-2 transition hover:bg-white sm:flex"><Avatar user={user} size="sm" /><div className="hidden text-left xl:block"><strong className="block max-w-40 truncate text-xs text-slate-800">{user.fullName}</strong><span className="text-[11px] text-slate-500">{user.profileLabel}</span></div></Link></div>
          </div>
        </header>
