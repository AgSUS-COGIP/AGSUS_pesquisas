...
const mainItems: NavItem[] = [
  { href: "/area", label: "Visão geral", icon: "⌂", module: "HOME" },
...
const workItems: NavItem[] = [
  ...
  { href: "/resultados", label: "Resultados", icon: "◔", module: "RESULTS" },
];
const adminItems: NavItem[] = [
  { href: "/admin", label: "Central administrativa", icon: "◆", module: "ADMIN_SURVEYS" },
...
function Avatar({ user, size = "md" }: { user: PlatformUser; size?: "sm" | "md" }) {
  ...
  return user.avatarUrl ? (
    ...
    <div className={`grid ${dimensions} shrink-0 place-items-center bg-[linear-gradient(135deg,#ffffff,#cce9ff)] font-black text-[#003b70] shadow-sm`}>
...
function NavGroup({ title, items, modules, compact, onNavigate }: { title: string; items: NavItem[]; modules: string[]; compact: boolean; onNavigate?: () => void }) {
  ...
  return <div className="mt-6">
    ...
    <nav className="mt-2 space-y-1.5">{allowed.map((item) => {
      const active = pathname === item.href || (item.href !== "/area" && pathname.startsWith(`${item.href}/`));
      return <Link key={item.href} href={item.href} onClick={onNavigate} title={compact ? item.label : undefined}
        className={`flex items-center gap-3 rounded-xl px-3 py-3 text-sm font-extrabold transition ${active ? "bg-white text-[#003b70] shadow-sm" : "text-blue-50 hover:bg-white/10"}`}>
        <span className="grid h-7 w-7 shrink-0 place-items-center text-base">{item.icon}</span>{!compact && <span>{item.label}</span>}
      </Link>;
    })}</nav>
  </div>;
}
...
function Sidebar({ user, compact, modules, mobile, onNavigate, onToggle, onSignOut }: { user: PlatformUser; compact: boolean; modules: string[]; mobile?: boolean; onNavigate?: () => void; onToggle?: () => void; onSignOut: () => void }) {
  return <aside className={`${mobile ? "fixed inset-y-0 left-0 z-[70] flex w-72" : `fixed inset-y-0 left-0 z-50 hidden lg:flex ${compact ? "w-20" : "w-64"}`} flex-col bg-[linear-gradient(180deg,#003b70,#002c54)] text-white shadow-2xl transition-all`}>
    ...
    {!mobile && <button type="button" onClick={onToggle} className="absolute -right-4 top-24 grid h-8 w-8 place-items-center rounded-full border border-blue-200 bg-white font-black text-[#003b70] shadow-md" aria-label={compact ? "Expandir menu" : "Recolher menu"}>{compact ? ">" : "<"}</button>}
    ...
    <div className="border-t border-white/10 p-3"><div className={`rounded-2xl border border-white/15 bg-white/10 p-3 ${compact && !mobile ? "text-center" : ""}`}>
      <Link href="/perfil" onClick={onNavigate} className={`flex items-center gap-3 ${compact && !mobile ? "justify-center" : ""}`}><Avatar user={user} size="sm" />{(!compact || mobile) && <div className="min-w-0"><strong className="block truncate text-sm">{user.fullName}</strong><span className="text-xs text-blue-200">{user.profileLabel}</span></div>}</Link>
      <button type="button" onClick={onSignOut} className="mt-3 w-full rounded-xl border border-white/20 px-3 py-2 text-sm font-bold transition hover:bg-white/10">{compact && !mobile ? "↪" : "Sair do sistema"}</button>
    </div></div>
  </aside>;
}
...
export function PlatformShell({ user, title, eyebrow, children, actions }: { user: PlatformUser; title: string; eyebrow?: string; children: ReactNode; actions?: ReactNode }) {
  ...
  return <main className="min-h-screen bg-[#f2f6fa] text-[#10243e]">
    <Sidebar user={user} compact={compact} modules={modules} onToggle={() => setCompact((v) => !v)} onSignOut={signOut} />
    ...
    <div className={`transition-all ${compact ? "lg:pl-20" : "lg:pl-64"}`}>
      <header className="sticky top-0 z-40 border-b border-[#d7e5f2] bg-white/95 px-4 py-3 shadow-sm backdrop-blur lg:px-7 lg:py-4">
        <div className="mx-auto flex max-w-[1500px] items-center justify-between gap-4">
          <div className="flex min-w-0 items-center gap-3"><button type="button" onClick={() => setMobileOpen(true)} className="grid h-10 w-10 place-items-center rounded-xl border border-slate-200 bg-white text-xl font-black text-[#003b70] lg:hidden">☰</button><div className="min-w-0">{eyebrow && <p className="text-[10px] font-black uppercase tracking-[0.18em] text-[#0b8f58]">{eyebrow}</p>}<h1 className="truncate text-xl font-black text-[#003b70] sm:text-2xl">{title}</h1></div></div>
      ...
      <div className="mx-auto max-w-[1500px] px-4 py-5 sm:px-5 lg:px-7 lg:py-7">{children}</div>
    </div>
  </main>;
}
