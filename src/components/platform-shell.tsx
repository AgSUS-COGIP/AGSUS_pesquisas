...
function Avatar({ user, compact = false }: { user: PlatformUser; compact?: boolean }) {
  ...
  if (user.avatarUrl && !failed) {
    return <span className={`${dimensions} grid shrink-0 place-items-center overflow-hidden bg-white ring-1 ring-slate-200`}><img src={user.avatarUrl} onError={() => setFailed(true)} alt={`Avatar de ${user.fullName}`} className="h-full w-full object-cover" referrerPolicy="no-referrer" /></span>;
  }
  return <span className={`${dimensions} grid shrink-0 place-items-center bg-sky-50 text-sm font-black text-[#003b70] ring-1 ring-sky-100`}>{initials(user.fullName)}</span>;
}
...
export function PlatformShell({ user, title, eyebrow, children, actions }: { user: PlatformUser; title: string; eyebrow?: string; children: ReactNode; actions?: ReactNode }) {
  ...
  const resolvedUser = useMemo(() => ({ ...user, avatarUrl: user.avatarUrl || authAvatarUrl }), [user, authAvatarUrl]);
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      const candidate = metadata.avatar_url ?? metadata.picture ?? metadata.photo_url;
      if (active && typeof candidate === "string" && candidate.trim()) setAuthAvatarUrl(candidate.trim());
    };
    void load();
    return () => { active = false; };
  }, [user.avatarUrl]);
