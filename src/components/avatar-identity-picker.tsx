...
function generatedAvatar(name: string, variant: number) {
...
export function AvatarIdentityPicker({ personName, currentUrl, currentSource, googleUrl }: Props) {
  ...
  const generated = useMemo(() => Array.from({ length: 8 }, (_, index) => generatedAvatar(personName, avatarRound * 8 + index)), [personName, avatarRound]);
  ...
  const activePreview = selectedUrl ? <img src={selectedUrl} alt="Imagem em uso" className="h-full w-full object-cover" referrerPolicy="no-referrer" /> : <span className="text-3xl font-bold text-slate-700">{initials(personName)}</span>;
  ...
  return <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
    <div className="grid lg:grid-cols-[280px_1fr]">
      <aside className="border-b border-slate-200 bg-slate-50 p-6 lg:border-b-0 lg:border-r">
        ...
        <div className="mx-auto mt-6 grid h-40 w-40 place-items-center overflow-hidden rounded-full bg-white ring-1 ring-slate-200 shadow-sm">{activePreview}</div>
        <h3 className="mt-5 text-center text-lg font-semibold text-slate-950">{personName}</h3>
        <p className="mt-1 text-center text-sm text-slate-500">Origem: {selectedSource === "GOOGLE" ? "Conta Google" : selectedSource === "UPLOADED" ? "Foto enviada" : selectedSource === "GENERATED" ? "Avatar profissional" : "Iniciais"}</p>
      </aside>

      <div className="p-6 sm:p-7">
        <div>
          <h2 className="text-xl font-semibold tracking-tight text-slate-950">Escolha como deseja aparecer</h2>
          <p className="mt-2 text-sm leading-6 text-slate-600">Uma única escolha é aplicada em toda a plataforma. Ela não será substituída automaticamente em novos acessos.</p>
        </div>
        ...
        <div className="mt-6 grid gap-3 sm:grid-cols-3">
          <button type="button" disabled={!googleUrl || Boolean(saving)} onClick={() => void persist("GOOGLE")} className={`choice-card ${selectedSource === "GOOGLE" ? "choice-card-active" : ""}`}>
            <span className="choice-icon"><ImageIcon className="h-5 w-5" /></span><span><strong>Foto do Google</strong><small>{googleUrl ? "Usar a foto da conta institucional" : "Foto não disponibilizada"}</small></span>{selectedSource === "GOOGLE" && <Check className="ml-auto h-4 w-4" />}
          </button>
          <button type="button" disabled={Boolean(saving)} onClick={() => inputRef.current?.click()} className={`choice-card ${selectedSource === "UPLOADED" ? "choice-card-active" : ""}`}>
            <span className="choice-icon"><Camera className="h-5 w-5" /></span><span><strong>Enviar foto</strong><small>JPG, PNG ou WEBP até 5 MB</small></span>{saving === "UPLOAD" ? <Loader2 className="ml-auto h-4 w-4 animate-spin" /> : selectedSource === "UPLOADED" && <Check className="ml-auto h-4 w-4" />}
          </button>
          <button type="button" disabled={Boolean(saving)} onClick={() => void persist("INITIALS")} className={`choice-card ${selectedSource === "INITIALS" ? "choice-card-active" : ""}`}>
            <span className="choice-icon"><UserRound className="h-5 w-5" /></span><span><strong>Usar iniciais</strong><small>Opção simples e neutra</small></span>{selectedSource === "INITIALS" && <Check className="ml-auto h-4 w-4" />}
          </button>
        ...
        <div className="mt-8 flex flex-wrap items-center justify-between gap-3 border-t border-slate-200 pt-6">
          <div><h3 className="font-semibold text-slate-950">Avatares profissionais</h3><p className="mt-1 text-sm text-slate-500">Retratos vetoriais mais sóbrios, adequados ao ambiente institucional.</p></div>
          <button type="button" onClick={() => setAvatarRound((value) => value + 1)} className="secondary-button"><RotateCw className="h-4 w-4" />Gerar outras opções</button>
        </div>
        <div className="mt-4 grid grid-cols-4 gap-3 sm:grid-cols-8">
          {generated.map((url, index) => <button key={`${avatarRound}-${index}`} type="button" disabled={Boolean(saving)} onClick={() => void persist("GENERATED", url)} className="group relative aspect-square overflow-hidden rounded-full bg-slate-100 ring-1 ring-slate-200 transition hover:-translate-y-0.5 hover:ring-2 hover:ring-blue-500"><img src={url} alt={`Avatar profissional ${index + 1}`} className="h-full w-full object-cover" />{selectedSource === "GENERATED" && selectedUrl === url && <span className="absolute inset-0 grid place-items-center bg-blue-950/35 text-white"><Check className="h-5 w-5" /></span>}</button>)}
        </div>
      </div>
    </div>
  </section>;
