...
| `survey-cycle-period.ts` | `periodIssues()`, `publishBlockedMessage()`, `reopenIssue()`, `nowLocalInputValue()`, tipo `PeriodIssue` |
...
Serializa gravações encadeando `tail`. `enqueue(operacao, chave?)` devolve a promise da operação (para o chamador tratar erro) e mantém `tail` sempre resolvida, de modo que uma falha não trava a fila. As falhas são rastreadas **por chave** (as telas passam o id da pergunta): o sucesso de uma gravação só limpa a falha da mesma chave — sem isso, salvar a pergunta B mascararia a falha da pergunta A e o envio definitivo passaria com resposta ausente. `getSnapshot()` deriva `SAVING`/`ERROR`/`IDLE` de `pending` e das falhas pendentes; `subscribe()` emite imediatamente ao assinar. `flush()` aguarda a fila e relança a falha mais recente ainda não resolvida.
...
- `survey-catalog.ts` e `reliable-save-queue.ts` saíram do limbo: `/area` e `/pesquisas` consomem o catálogo (pelo hook `@/hooks/use-survey-catalog`) e as duas jornadas do CDDI **e** o runtime genérico (`/pesquisas/[applicationCode]`) usam `ReliableSaveQueue` — a fila própria com `useRef<Promise>` do runtime foi eliminada.
